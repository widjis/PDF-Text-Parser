...
class FileOrganizer {
  ...
  async createDownloadableZipFromCached(classificationResults, sourceFiles) {
    try {
      ...
      if (!Array.isArray(classificationResults) || !Array.isArray(sourceFiles)) {
        throw new Error('Invalid input: classificationResults and sourceFiles must be arrays');
      }
      ...
      // Create temporary files from base64 data and build file mapping
      const fileMapping = {};
      for (let i = 0; i < sourceFiles.length; i++) {
        const sourceFile = sourceFiles[i];
        
        // Validate sourceFile structure
        if (!sourceFile || !sourceFile.filename) {
          console.warn(`[FileOrganizer] Skipping invalid source file at index ${i}: missing filename`);
          continue;
        }
        
        if (!sourceFile.base64Data) {
          console.warn(`[FileOrganizer] Skipping file ${sourceFile.filename}: missing base64Data`);
          continue;
        }
        ...
        try {
          ...
          // Add to file mapping
          fileMapping[sourceFile.filename] = tempFilePath;
          
        } catch (bufferError) {
          console.error(`[FileOrganizer] Error processing file ${sourceFile.filename}:`, bufferError);
          continue;
        }
      }
      
      console.log('[FileOrganizer] Created', Object.keys(fileMapping).length, 'temporary files');
      
      if (Object.keys(fileMapping).length === 0) {
        throw new Error('No valid files were processed from the source data');
      }
      ...
      // Create ZIP file
      const zipResult = await this.createZipFromOrganizedFiles(organizationResult.tempDir);
  ...
  /**
   * Create ZIP file from organized directory
   * @param {string} sourceDir - Directory to zip
   * @returns {Promise<Object>} ZIP creation result with download info
   */
  async createZipFromOrganizedFiles(sourceDir) {
    try {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const zipId = `zip_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const zipFilename = `organized_documents_${timestamp}.zip`;
      
      // Create temp downloads directory (matching server.js expectation)
      const downloadDir = path.join(__dirname, '..', 'temp', 'zips');
      if (!fs.existsSync(downloadDir)) {
        fs.mkdirSync(downloadDir, { recursive: true });
      }
      
      // Use zipId as the actual filename for the download endpoint to find
      ...
      if (zipResult.success) {
        return {
          success: true,
          zipPath: zipPath,
          zipFilename: zipFilename, // Keep the user-friendly filename for display
          zipId: zipId,
          filename: zipFilename, // Add this for consistency with server response
      ...
      console.error('Error creating ZIP from organized files:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async organizeFilesForDownload(classificationResults, fileMapping, tempDir = null) {
    const baseDir = tempDir || `./temp_organize_${Date.now()}`;
    // Create organized files in a separate subdirectory to avoid including temp files in ZIP
    const outputDir = path.join(baseDir, 'organized');
    const results = {
      success: true,
      organized: [],
      failed: [],
      summary: {},
      tempDir: outputDir // Return the organized subdirectory path for ZIP creation
    };
    ...
    try {
      // Create temporary folder structure in the organized subdirectory
      const folderResult = await this.createFolderStructure(outputDir);
      if (!folderResult.success) {
        throw new Error('Failed to create folder structure: ' + folderResult.error);
      }
