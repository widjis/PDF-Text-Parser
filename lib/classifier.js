...
class DocumentClassifier {
  ...
  buildClassificationPrompt(text = '', filename) {
    const basePrompt = `Klasifikasikan dokumen bisnis Indonesia ini ke dalam salah satu kategori berikut:
...
6. OOPR - Out of Policy Request (Exception/special requests)
7. SRF - SRF Scan (Service Request Forms)
8. DO - Skip (Delivery orders - mark as skip)
  ...
  parseClassificationResponse(response) {
    try {
      ...
      if (!response) {
        ...
        return { category: 'OOPR', confidence: 0.1 };
      }
      ...
      console.log('🔍 [PARSER] Searching for KATEGORI and CONFIDENCE...');
      ...
      console.log(`🔍 [PARSER] Before validation - Category: ${category}, Confidence: ${confidence}`);
      ...
      const finalResult = { category, confidence };
      ...
      const fallbackResult = { category: 'OOPR', confidence: 0.1 };
